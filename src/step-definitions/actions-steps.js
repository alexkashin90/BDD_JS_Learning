...
When('I click add new doctor button from list header', function () {
    return pages('doctors').doctorListHeader.addNewDoctorButton.click();
});
...
When('I enter {string} into the {string} input field', function (value, inputField) {
    return pages('doctors').addDoctorModal.enterValue(value, inputField);
});